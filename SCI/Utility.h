...
namespace SCI {
    ...
    */
    ...
    // This function quantizes the value val [min_v, max_v] to [0,range-1]
    inline int Quantize( float val, float min_v, float max_v, int range ){
        float t = (val-min_v)/(max_v-min_v);
        int ret = (int)(t*(float)range);
        if( ret >= range ) return range-1;
        if( ret < 0 ) return 0;
        return ret;
    }
